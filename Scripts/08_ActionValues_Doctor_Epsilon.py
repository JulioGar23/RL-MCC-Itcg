...
# Greedy action selection using epsilon-greedy strategy.
...
epsilon = 0.1
q_values = [0.0, 0.0, 0.0]     # With q_values initialized to, the agent git stuck always choosing the first bottle
...
for t in range(1, 2500): # 5 iterations
    if random.random() < epsilon:
        selected_index = random.randint(0, 2)   # Explore: Random action selection (0, 1, or 2)
    else:
